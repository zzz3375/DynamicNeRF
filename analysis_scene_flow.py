...
consider the training process in run_nerf.py
Trained a dynamic nerf model based on configs/config-WTB-Beijing-no_combine.txt
write a python script from the argparser in run_nerf.py (reduce default chunk to 16384 and --netchunk to 16384)
load the trained model with the last iteration and set no_grad
for the first frame
1. show the first image as numpy array by cv2 window
2. I manual select a branch of pixels (N_p),  when select one pixel, draw red cycle centered at the selected pixel with 2 pixel radiance in the cv2 window (manual interaction)
3. get the surface 3D coordinate (pt_3d, [N_p, 3]) by the depth maps from the sampling ray of selected pixel

for the N_step-1 consequtive frames (use tqdm bar)
4. trace the scene_flow of pt_3ds along all time_step, as a scene_flow-step relation [N_p, N_step, 3], the last do not have forward flow, set to 0
5. cumsum the scene_flow to get the displace-step plot [N_p, N_step, 3], the first step set displacement 0, plot as matplotlib, 
6. save as pd dataframe to ./scene_flow_analysis/{dataset_name}/displacement_x.csv displacement_y.csv displacement_z.csv with columns [Time (s),Sensor 1,Sensor 2,Sensor 3,Sensor 4], interval of time step is 1/30 second
...
def main():
    ...
    with torch.no_grad():
        ret = render(0.0 / float(num_img) * 2. - 1.0, False, H, W, focal, chunk=args.chunk, c2w=first_pose, **render_kwargs_test)
